...
class TSPVisualizer:
    def __init__(self):
        ...
        pygame.display.set_caption("TSP Visualizer - 旅行推銷員問題")
    ...
    def init_ui(self):
        ...
        self.buttons = [
            {"text": "Random Cities", "width": btn_width, "action": "random"},
            {"text": "Clear All", "width": btn_width, "action": "clear"},
            {
                "text": "Start",
                "width": btn_width // 2 - 5,
                "action": "start",
            ...
            {"text": "Reset", "width": btn_width, "action": "reset"},
        ]
        ...
        if canvas_rect.collidepoint(pos):
            for city_id, city_pos in self.cities.items():
                ...
                if math.sqrt(dx * dx + dy * dy) <= config.CITY_RADIUS:
                    ...
                    self.tooltip = (f"City {city_id}: {city_pos}", pos)
                    break
    ...
    def renumber_cities(self):
        """Renumber city IDs to be consecutive starting from 0"""
        if not self.cities:
            return
    ...
    def update_slider_value(self, slider, x, width):
        ...
        if slider["label"] == "Speed":
            ...
            if self.animation_speed < 100:
                self.iterations_per_step = self.animation_speed * 10
            else:
                self.iterations_per_step = self.animation_speed * 10
    ...
    def draw(self):
        ...
        if len(self.cities) >= 2:
            ...
            self.renderer.draw_info_text(
                "Click to add cities, or use 'Random Cities' button",
        ...
        if len(self.cities) >= 2:
            info_y = 10
            self.renderer.draw_info_text(f"Cities: {len(self.cities)}", (10, info_y))
            info_y += 20
            self.renderer.draw_info_text(
                f"Current Distance: {self.current_distance:.2f}", (10, info_y)
            )
            info_y += 20
            self.renderer.draw_info_text(
    ...
    def get_stats(self):
        return {
            ...
            "Best Distance": f"{self.best_distance:.2f}",
            "Iterations": len(self.chart_points),
