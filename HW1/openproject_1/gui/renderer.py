...
class Renderer:
    ...
    def draw_control_panel(self, buttons, sliders, stats, algorithm_buttons):
        ...
        title = title_font.render("TSP Visualizer", True, config.TEXT_COLOR)
        ...
        algo_label = config.get_font(18).render("Algorithm:", True, config.TEXT_COLOR)
        ...
        stats_label = config.get_font(18).render("Statistics:", True, config.TEXT_COLOR)
    ...
    def draw_chart(self, points, chart_x, chart_y, chart_width, chart_height):
        ...
        dist_label = label_font.render(
            f"Best: {min_dist:.2f}", True, config.TEXT_SECONDARY_COLOR
        )
        self.screen.blit(dist_label, (inner_x, chart_y + 2))

        iter_label = label_font.render(
            f"Iter: {points[-1]['iteration']}", True, config.TEXT_SECONDARY_COLOR
        )
